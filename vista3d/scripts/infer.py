...
class InferClass:
    def __init__(self, config_file="./configs/infer.yaml", **override):
        ...
        logging.debug("🚀 Moving model to GPU...")
        self.model = model.to("cuda:0")
        logging.debug(f"✅ Model is on: {next(self.model.parameters()).device}")


        logging.debug(f"📂 Loading checkpoint from {ckpt_name}...")
    ...
    def infer(
        ...
        self.model.eval()
        logging.debug(f"🖼 Image file: {image_file}")
        logging.debug(f"🎯 Label prompt: {label_prompt}")
    ...
    def infer_everything(self, image_file, label_prompt=EVERYTHING_PROMPT, rank=0):
        ...
        for _device_in, _device_out in zip(device_list_input, device_list_output):
            try:
                ...
                logging.debug("🧹 Cleaning up GPU memory...")        
                torch.cuda.empty_cache()
                logging.debug("✅ GPU memory cleaned up!")
                batch_data = [
                    self.post_transforms(i) for i in decollate_batch(batch_data)
                ]
                logging.debug("💾 Saving output to disk...")
                batch_data = [self.save_transforms(i) for i in batch_data]
                logging.debug("✅ Output saved successfully!")
                finished = True
            except RuntimeError as e:
                if not any(x in str(e).lower() for x in ("memory", "cuda", "cudnn")):
                    raise e
